...
// gets the precomputed 9 bit orientation matrix
// #define GET_ORIENTATION_MATRIX

#ifdef GET_ORIENTATION_MATRIX
  #define DATA_LENGTH 79
#else
  #define DATA_LENGTH 43
#endif
...
int main(int argc, char** argv)
{
  ...
  if (reply[2] != '\x01')
    ...
    {
      ROS_INFO("Active mode set successfully: Rakesh and Sicong");
    }

  // ------------------ Setting sampling rate start ----------------------------- //
  // Read the sampling rate first
  char set_sampling_rate[20] = {
    0xDB, 0xA8, 0xB9, 
    0x00, // don't change, just reply
    0x00, 0x00, // decimation
    0x00, 0x00, 0, 0, 0, 0, 0, 0, 
    // Low Mag Power/Bandwidth
    1, 
    // reserved
    0, 0, 0, 0, 0
  };
  unsigned char reply_sampling_rate[19];
  ...
  set_sampling_rate[5] = 5;
  for (int i = 6; i < 14; i++) {
    set_sampling_rate[i] = reply_sampling_rate[i - 3];
  }
  boost::asio::write(*serial_port, boost::asio::buffer(set_sampling_rate, 20));
  boost::asio::read(*serial_port, boost::asio::buffer(reply_sampling_rate, 19));
...
//  ros::Time t0 = ros::Time::now();  
  ...
  unsigned short data_length = DATA_LENGTH;
